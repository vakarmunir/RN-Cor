...
function insertMenuItem(&$template, &$vars, &$parameters)
{
	...
	if ($item->parent_id) {
		...
		$query->select('m.path, m.level');
		$query->from('#__menu AS m');
		$query->where('m.id = ' . (int) $item->parent_id);
		$db->setQuery($query);
	...
	$query->from('#__menu AS m');
	$query->where('m.menutype = ' . $db->q($item->menutype));
	$query->where('m.parent_id = ' . (int) $item->parent_id);
	$query->order('m.ordering DESC');
	$db->setQuery($query);
	...
	$query->where('e.element = ' . $db->q($template->urlparams->option));
	$db->setQuery($query);
...
function renderHTML(&$template)
{
	if (isset($template->dbselect->table)) {
		...
		$query = $db->getQuery(true);
		$query->select('t.' . $template->dbselect->name . ' AS name');
		$query->select('t.' . $template->dbselect->alias . ' AS alias');
		$query->from($template->dbselect->table . ' AS t');
		foreach ($template->dbselect->where as $key => $val) {
			$query->where('t.' . $key . ' = ' . $db->q(getVar($val)));
		}
		$db->setQuery($query);
		$item = $db->loadObject();
	} else {
...
function getMenuItems($name, $width = '100%')
{
	...
	$query = $db->getQuery(true);
	$query->select('m.id, m.parent_id, m.title, m.menutype, m.type, m.published');
	$query->from('#__menu AS m');
