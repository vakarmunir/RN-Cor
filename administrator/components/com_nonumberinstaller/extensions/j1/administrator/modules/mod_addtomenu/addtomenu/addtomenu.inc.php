...
if (!$app->isSite() && $user->get('guest') || !$user->authorize('com_content', 'edit', 'content', 'all')) {
...
if ($lang->getTag() != 'en-GB') {
	...
	$lang->load($option . '.menu', JPATH_ADMINISTRATOR, 'en-GB');
}
...
$lang->load($option . '.menu', JPATH_ADMINISTRATOR, null, 1);
...
function insertMenuItem(&$template)
{
	...
	$query = 'SELECT `id`'
		...
		. ' WHERE `link` <> ' . $db->quote('')
		. ' AND `parent` = 0'
		. ' AND `option` = ' . $db->quote($template->urlparams->option)
		. ' LIMIT 1';
	$db->setQuery($query);
	...
	if ($error) {
		...
		$error = JText::_('ERROR SAVING MENU ITEM') . ': ' . $error;

		JError::raiseWarning('1', $error);
		renderHTML($template);
	} else {
		echo "<script> window.parent.addtomenu_setMessage( '" . addslashes(JText::_('MENU ITEM SAVED')) . "', 1 ); </script>\n";
	}
}
...
function renderHTML(&$template)
{
	if (isset($template->dbselect->table)) {
		...
		foreach ($template->dbselect->where as $key => $val) {
			$val = getVar($val);
			$where[] = 't.' . $key . ' = ' . $db->quote($val);
		}

		$query = 'SELECT '
			. 't.' . $template->dbselect->name . ' as name, '
			. 't.' . $template->dbselect->alias . ' as alias'
			. ' FROM ' . $template->dbselect->table . ' AS t'
...
<form action="<?php echo $uri->toString(); ?>" method="post" name="adminForm" id="adminForm">
	...
	<div id="toolbar-box" class="nn-toolbar-box">
		...
		<div class="m">
			...
			<div class="header">
				<small>
					<small><?php echo JText::_('ADD_TO_MENU'); ?></small>
				</small>
				<br />
