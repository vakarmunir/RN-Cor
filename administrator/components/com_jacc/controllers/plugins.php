<?php
/**
* @version		$Id: plugins.php 109 2011-08-14 12:13:08Z michel $
* @package		Jacc
...
class JaccControllerPlugins extends JaccController
{
	...
	public function zipLivePlugin($model)
	{
		...
		if ($jv->RELEASE > 1.5) {
			...
			//Joomla 1.5 doesn't use folders for the plugins 
			$files = JFolder::files(JPATH_SITE.DS.'plugins'.DS.$item->folder, $item->name, true, true);
			if (count($files)) {
				foreach ($files as $file) {
					JFile::copy($file, $model->getTempPath().DS.JFile::getName($file));
				}
				//Some plugins uses subfolders	
				if (JFolder::exists(JPATH_SITE.DS.'plugins'.DS.$item->folder.DS.$item->name)) {
					JFolder::copy(JPATH_SITE.DS.'plugins'.DS.$item->folder.DS.$item->name, $model->getTempPath().DS.$item->name,'',true);
				}
			} else {
				JFolder::delete($model->getTempPath());
				return -2;
