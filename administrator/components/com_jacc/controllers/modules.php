...
class JaccControllerModules extends JaccController
{
	...
	public function zipLiveModule($model)
	{
		...
		//Copy the component
		if (JFolder::exists(JPATH_SITE.DS.'modules'.DS.$item->name)) {
		    JFolder::copy(JPATH_SITE.DS.'modules'.DS.$item->name, $model->getTempPath(),'',true);
		} elseif (JFolder::exists(JPATH_ADMINISTRATOR.DS.'modules'.DS.$item->name)) {						      
		    JFolder::copy(JPATH_ADMINISTRATOR.DS.'modules'.DS.$item->name, $model->getTempPath(),'',true);
		} else {
	...
	public function zip () {
		...
		JFolder::copy(JPATH_COMPONENT.DS.'templates'.DS.'modules', $model->getTempPath());
