...
$javascript_array_downloads = 'var downloads = new Array(';
$first = true;
foreach($this->items as $download){		
	if($first){
		$first = false;
	}else{
		$javascript_array_downloads .= ',';
	}
	$javascript_array_downloads .= "'".$download->id."'";
}	
$javascript_array_downloads .= ');';
...
<?php echo $javascript_array_downloads."\n"; ?>

function select_all(usergroup_id, select_all_id){
	action = document.getElementById(select_all_id).checked;	
	for (i = 0; i < downloads.length; i++){
		box_id = downloads[i]+'__'+usergroup_id;
		hidden_id = downloads[i]+'__'+usergroup_id+'__hidden';
		if(action==true){
...
Joomla.submitbutton = function(task){
	if (task == 'download') {			
		document.location.href = 'index.php?option=com_frontenduseraccess&view=download&sub_task=new';		
	}		
	if (task == 'download_delete') {
	...
	if (task == 'access_downloads_save') {
		submitform('access_downloads_save');
	}
}
...
<form name="adminForm" method="post" action="<?php echo JRoute::_('index.php?option=com_frontenduseraccess&view=downloads'); ?>">	
...
<table class="adminlist">
	<tr>	
		...
		<th align="left">				
			...
			echo JHTML::_('grid.sort', $label, 'd.name', $listDirn, $listOrder); 			
			?>				
		</th>	
		<?php				
	...
	<?php
		...
		foreach($this->items as $download){	
			...
			echo '<tr class="row'.$k.'">';
			echo '<td><input type="checkbox" id="cb'.$counter.'" name="cid[]" value="'.$download->id.'" onclick="isChecked(this.checked);" /></td>';
			echo '<td class="column_ids">'.$download->id.'</td>';			
			echo '<td>';			
			echo '<a href="index.php?option=com_frontenduseraccess&view=download&id='.$download->id.'">';	
			if($download->description!=''){
				echo '<label class="hasTip" title="'.$download->name.'::'.$download->description.'">';
			}			
			echo $download->name;
			if($download->description!=''){
				echo '</label>';
			...
			foreach($this->fua_usergroups as $fua_usergroup){
				$checked = '';
				$checked_hidden = '';
				if (in_array($download->id.'__'.$fua_usergroup->id, $this->access_downloads)) {
					$checked = 'checked="checked"';
					$checked_hidden = '1';
				}
				echo '<td style="text-align:center;"><input type="hidden" name="download_access_hidden[]" id="'.$download->id.'__'.$fua_usergroup->id.'__hidden" value="'.$download->id.'__'.$fua_usergroup->id.'__hidden__'.$checked_hidden.'" /><input type="checkbox" name="part_access[]" id="'.$download->id.'__'.$fua_usergroup->id.'" onclick="toggle_right(\''.$download->id.'__'.$fua_usergroup->id.'__hidden\');" value="'.$download->id.'__'.$fua_usergroup->id.'" '.$checked.' /></td>';
			}
			echo '</tr>';			
			if($counter==7){
				echo '<tr><th>&nbsp;</th><th>&nbsp;</th>';					
