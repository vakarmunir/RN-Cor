...
defined('_JEXEC') or die('Restricted access');
  ...
  JToolBarHelper::title( JText::_( 'Role' ), 'generic.png' );
...
<form action="index.php?option=com_rednet&amp;view=role" method="post" name="adminForm">
	<table>
		<tr>
...
<?php
  ...
  for ($i=0, $n=count( $this->items ); $i < $n; $i++):
    ...
    if (JRequest::getVar('function', null)) {
    	$onclick= "onclick=\"window.parent.jSelectRole_id('".$row->id."', '".$this->escape($row->name)."', '','id')\" ";
    }  	
    
 	$link = JRoute::_( 'index.php?option=com_rednet&view=role&task=edit&cid[]='. $row->id );
 	$row->id = $row->id; 	
