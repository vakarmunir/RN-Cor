...
require_once(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_rednet'.DS.'helpers'.DS.'query.php');
...
class RednetHelper
{
	 ...
	 */
	public static function addSubmenu($vName = 'role')
	{
        
		JSubMenuHelper::addEntry(
			JText::_('Role'),
			'index.php?option=com_rednet&view=role',
			($vName == 'role')
		);

		JSubMenuHelper::addEntry(
			JText::_('Workers'),
			'index.php?option=com_rednet&view=workers',
			($vName == 'workers')
		);

		JSubMenuHelper::addEntry(
			JText::_('Testvc'),
			'index.php?option=com_rednet&view=testvc',
			($vName == 'testvc')
		);
	...
	public static function getExtensions() 
	{
		...
		$jv = new JVersion();
		$alt_libdir = ($jv->RELEASE < 1.6) ? JPATH_ADMINISTRATOR.DS.'components'.DS.'com_rednet' : null;
		JLoader::import('joomla.utilities.xmlelement', $alt_libdir);
		
		$xml = simplexml_load_file(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_rednet'.DS.'elements'.DS.'extensions.xml', 'JXMLElement');		        
...
abstract class JHtmlRednet
{
	...
	{
		$extensions = RednetHelper::getExtensions();
		$options = array();
		...
		foreach ($extensions as $extension) {   
			...
			$option->value = 'com_rednet.'.$extension->name;
			$options[] = $option;			
		}		
	...
	public static function categories($extension, $cat_id,$name="categories",$title="Select Category", $config = array('attributes'=>'class="inputbox"','filter.published' => array(0,1)))
	{
			...
			$query->from('#__rednet_categories AS a');
